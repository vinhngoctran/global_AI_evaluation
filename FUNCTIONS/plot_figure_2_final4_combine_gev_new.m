function plot_figure_2_final4_combine_gev_new()
%%
clc; close all; 
...
load('RESULTS_FINAL/R4_2_NearingHydrological_final_GEV.mat','RP','checkdata','FloodFrequency','FloodFrequency_delta','PE','T2P','StartEndYear','Peak_FFA');

load Col.mat % Color setup

figure1 = figure('OuterPosition',[300 100 1200 900]);
axes1 = axes('Parent',figure1,...
    'Position',[0.07 0.65 0.25 0.25]);hold on;
...
for j=1:size(Peak_FFA,1)
    if NSEmark(j,2)==0
        for i=1:5
            if ~isempty(Peak_FFA{j,i})
            PE_FFA{1,i} = [PE_FFA{1,i};Peak_FFA{j,i}];
            end
        end
    elseif NSEmark(j,2)==1
        for i=1:5
            if ~isempty(Peak_FFA{j,i})
            PE_FFA{2,i} = [PE_FFA{2,i};Peak_FFA{j,i}];
            end
        end
    end
end
    for j=1:5
        FD_nat(1:size(PE_FFA{1,j},1),j) = PE_FFA{1,j};
        FD_hum(1:size(PE_FFA{2,j},1),j) = PE_FFA{2,j};
    end
FD_nat(FD_nat==0)=NaN;
FD_nat(FD_nat==Inf)=NaN;
numel(find(~isnan(FD_nat(:,1))))

PCT = prctile(FD_nat,50)

FD_hum(FD_hum==0)=NaN;
FD_hum(FD_hum==Inf)=NaN;
...
b(1).BoxFaceColor = [0.1 0.1 0.1]; % Black for natural
...
for i=1:5
    text(i*2-0.75,median(FD_nat(:,i),'omitnan'),num2str(numel(find(~isnan(FD_nat(:,i))))),'Rotation',90,'HorizontalAlignment','center')
    text(i*2+0.75,median(FD_hum(:,i),'omitnan'),num2str(numel(find(~isnan(FD_hum(:,i))))),'Rotation',90,'HorizontalAlignment','center','Color',Col(2,:))
end
xlabel('Return period [year]'); ylabel('PE [%]')
legend(["","Natural","Dammed"],'box','off','Position',[0.16041666891946 0.91 0.183277024132376 0.0297397762130774],...
...
axes1 = axes('Parent',figure1,...
    'Position',[0.4 0.65 0.25 0.25]);hold on;
...
set(axes1,'Color','none','Linewidth',1,'FontSize',13);
...
title(['d '],'FontSize',15);axes1.TitleHorizontalAlignment = 'left';
...
exportgraphics(figure1,"Figure_final/F2_final.jpg",'Resolution',600)
% exportgraphics(figure1, "Figure_final/.pdf", 'ContentType', 'vector');
% close all
end