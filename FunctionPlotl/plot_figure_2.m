...
axes1 = axes('Parent',figure1,...
    'Position',[0.73 0.65 0.25 0.1]);hold on;
data = removeNaNRows(FET2P_Nat(:,2));
[f, xi] = ksdensity(data, 'Bandwidth', std(data)*(4/3/length(data))^0.2);
Med1 = median(data,'omitnan');
area(xi,f,'FaceColor',[0.5 0.5 0.5],'EdgeColor','k','FaceAlpha',0.5)

data = removeNaNRows(FET2P_Hum(:,2));
[f, xi] = ksdensity(data, 'Bandwidth', std(data)*(4/3/length(data))^0.2);
Med2 = median(data,'omitnan');
area(xi,f,'FaceColor',Col(2,:),'EdgeColor','b','FaceAlpha',0.3) 
set(axes1,'Color','none','Linewidth',0.5,'FontSize',13);
l1=plot(Med1,0,'Marker','o','MarkerEdgeColor','k','MarkerFaceColor',[0.5 0.5 0.5],'MarkerSize',20);
l2=plot(Med2,0,'Marker','^','MarkerFaceColor',Col(2,:),'MarkerEdgeColor','b','MarkerSize',20);
title("d",'FontSize',16);axes1.TitleHorizontalAlignment = 'left';
xlim([-10 10])
xlabel('T2P [day]');ylabel('PDF [-]')
LengThD = [10,20,30,41];
...
event1 = find(DATETIME==datetime(YYYY(1),1,1)+days(idx(1)));
event2 = find(DATETIME==datetime(YYYY(2),1,1)+days(idx(2)));
% EndT = StaT + days(30);


OBS_check(OBS_check==-999)=NaN;
...
ylim([0 200])
xlim([DATETIME(event1-15), DATETIME(event1+15)])
...
ylim([0 200])
xlim([DATETIME(event2-15), DATETIME(event2+15)])
axes1 = axes('Parent',figure1,...
...
% exportgraphics(figure1,"Figures2/F2.jpg",'Resolution',600)
% exportgraphics(figure1, "Figures2/F2.pdf", 'ContentType', 'vector');
% close all
end